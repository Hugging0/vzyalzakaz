...
function BootstrapScreen({ state, onRetry }: { state: BootstrapState; onRetry: () => void }) {
  ...
  return <main className="app-shell"><div className="app-content bootstrap"><AppCard><AlertCircle size={28} color="var(--warning)" /><h1>{unavailable ? "Откройте через Telegram" : "Не удалось открыть Hunt Agent"}</h1><p className="muted">{unavailable ? "Mini App защищён Telegram-авторизацией. Откройте его кнопкой в боте после настройки домена." : "Проверьте соединение и попробуйте ещё раз. Ваши данные не изменены."}</p>{!unavailable && <AppButton onClick={onRetry}><RefreshCw size={17} /> Повторить</AppButton>}</AppCard></div></main>;
}
...
export function HuntApp() {
  ...
  const authenticate = async () => {
    setBootstrap("loading"); telegramBridge.ready(); document.documentElement.dataset.theme = telegramBridge.getColorScheme();
  ...
  return <MiniAppShell activeTab={tab} onTabChange={(nextTab) => { setSelectedLead(null); setTab(nextTab); }}>{!profile.isActive && <AppNotice tone="warning">Агент на паузе: новые уведомления и действия остановлены.</AppNotice>}{content}</MiniAppShell>;
}
