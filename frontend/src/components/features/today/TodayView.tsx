  ...
  return (
    <>
      ...
      <section className="today-overview">
        <AppCard className="agent-panel" tone={profile.isActive ? "mint" : "yellow"}><div><span>Состояние агента</span><h2>{profile.isActive ? "Поиск работает" : "Поиск на паузе"}</h2><p>{profile.isActive ? `Уведомления приходят от ${profile.matchThreshold}% совпадения.` : "Новые рекомендации и уведомления остановлены."}</p></div><AppLinkButton href="/app/settings" variant="ghost">Настроить</AppLinkButton></AppCard>
