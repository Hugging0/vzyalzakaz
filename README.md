...
Production-сборка frontend автоматически упаковывает ту же версию расширения и
публикует архив по адресу `/downloads/vzyalzakaz-extension-chromium.zip`. Ссылка на
него доступна в PWA на экране «Площадки». Это тестовый канал: на macOS и Windows
пользователь распаковывает архив и загружает папку через режим разработчика Chrome
или Яндекс Браузера. Для установки в один клик и автоматических обновлений нужен
Chrome Web Store; Safari выпускается отдельной сборкой через App Store.
