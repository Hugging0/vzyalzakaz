...
Production scheduler опрашивает 16 Web / API / RSS-источников. Пользовательский MTProto collector авторизован, подключён через отдельный приватный SOCKS5 и слушает все 33 Telegram-канала. SOCKS5 разрешает подключения только с IP production VPS; Bot API, Web/PWA, LLM и остальные сервисы идут напрямую. Начальный backfill выполняется последовательно в фоне и не блокирует запуск API.
