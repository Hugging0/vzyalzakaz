...
В пользовательской ленте и деталях числовой ranking не показывается. Сортировка
«Рекомендуемые» использует backend-порядок без дополнительного клиентского порога.
Показывать конкретную причину рекомендации и условия для уточнения. Не заменять
старые числовые диапазоны новыми обещаниями пригодности. Диагностика доступна
администраторам, не занимает пользовательскую карточку.
...
- внутренний score не выводится в ленте, карточке и на главной;
- метод поиска и версия алгоритма доступны только технической диагностике;
- технические fact IDs доступны данным, но не заменяют человекочитаемую причину.

## 28. Лента и мобильные действия

- Главная начинает с рекомендаций, без декоративной статистики. Ошибка загрузки
  не выдаётся за отсутствие заказов.
- Лента и главная используют статус recommended; отклики находятся в своём разделе.
- Источник и сортировка сохраняются в URL; возврат из детали сохраняет выбор.
- Мобильное меню закрывается при любом переходе и повторном выборе текущего раздела.
- Деталь: бюджет, до трёх причин, до трёх уточнений, исходная задача и отклик.
- Одно основное действие; на мобильном закреплено над нижней навигацией и скрыто
  при открытом меню. Последний абзац можно прокрутить выше закреплённых элементов.
- Строки и детали заказов: спокойная поверхность, мягкий контур, radius 12px, без тени;
  заголовок основным шрифтом. Заголовок детали 28px/1.3, на узком экране 24px/1.3;
  подзаголовки 18px. Технические badges вроде API не выводятся.
- Денежные условия редактируются только в настройках: сумма за задачу отдельно
  от ставки в час. В профиле ссылка на этот раздел. Пустое поле снимает ограничение.
- AppButton по умолчанию type=button; формы явно задают type=submit для сохранения.
- --border-soft и --z-actions используются для тихих разделителей и панели действий.
